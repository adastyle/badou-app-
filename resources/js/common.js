...
// 七牛云地址
var qiniuRoot = 'http://qiniubadoupub.caihongjiankang.com/';
var projectMode='badou_prod';

var tokenExpire = false;

// 生产服务器地址
//var rootUri = 'http://www.badou001.cn/badou/r/';
// 测试服务器地址
var rootUri = 'http://test.gaokaobaike.cn/badou/r/';
// 本地开发地址
//var rootUri = 'http://192.168.1.116:8080/qidou/r/';

var mainTitleBarBackgroundColor = "#008FD3", // 主界面导航条和状态条颜色
	defaultTitleBarBackgroundColor = "#FFF", // 其他二级页面导航条和状态条颜色
	mainStatusBarStyle = 'dark', // 主界面状态条风格(文字颜色)
...
var SECURITY_API_Token = "cis*api*md5";
...
(function(mui, owner) {
	...
	owner.ajax = function(url, data, callback, options) {
		...
		// 安全机制
		if(isExitsFunction('md5')) {
			var rdm = Math.floor((Math.random()*1000000)+1);
			var cdate = formatDate(new Date(),'YY-MM-DD');
			var stk = md5(SECURITY_API_Token + rdm + cdate);
			xhr.setRequestHeader('random', rdm);
			xhr.setRequestHeader('token', stk);
			console.log('token:' + stk);
		}else
	 ...
	 * @param {Object} url
	 * @param {Object} options
	 * 	playCallback,pauseCallback,successCallback,progressCallback
	 */
	owner.audio = function(btnEle, url, options) {
		var options = options || {};
		var playCallback = options.playCallback;
		var pauseCallback = options.pauseCallback;
...
/**
	...
	owner.uploadFile = function(fps, callback, options) {
		...
		if(isExitsFunction('md5')) {
			...
			var cdate = formatDate(new Date(),'YY-MM-DD');
			var stk = md5(SECURITY_API_Token + rdm + cdate);
			task.setRequestHeader('random', rdm);
			task.setRequestHeader('token', stk);
		}
		osg.loading();
		task.start();
	}
	...
	//录像上传视频
	owner.uploadVideoFromCamera = function(maxNum, callback) {
	...
	owner.pickImageFromGallery = function(maxNum, callback) {
		...
		plus.gallery.pick(
			...
			function(e) {
				mui.toast('取消了选择'+e.message+e.code);
			}, {
				multiple: true,
				maximum: maxNum,
				filter: "image",
...
function myFunc(func, preScroll) {
	$()
		.ready(
			function() {
				if(!mui.os.ios && !mui.os.android) // 开发调试时初始化滚动条
				...
				mui.plusReady(function() {
					...
					if(immersed > 0) {
						...
						//	immersed += 10;
						var t = $('header');
						if(t.hasClass("hasSearch")){
								t.css('padding-top', (immersed+10) + 'px');
						...
						if(tn.attr('top') != 'true')
							...
							if(!tn.hasClass('mui-content'))
								tn.css('margin-top', (immersed + 49) + 'px');
							else {
						...
						if(td.attr('top') != 'true')
							...
							if(!td.hasClass('mui-content'))
								td.css('margin-top', (immersed + 49) + 'px');
							else {
